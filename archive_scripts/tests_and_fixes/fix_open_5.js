...
const assets = [
    { epic: 'CS.D.EURUSD.CFD.IP', type: 'forex' },
    { epic: 'CS.D.EURGBP.CFD.IP', type: 'forex' },
    { epic: 'IX.D.DOW.DAILY.IP', type: 'index' }
];

async function run() {
    console.log("📡 Atgūstu iepriekš atvērtās pozīcijas no IG...");
    ...
    for (const p of positions) {
        ...
        if(!cfg) continue; // Pārējiem jau fiksēti
        ...
        if(!activeTrades[`${epic}_REAL`]) {
            ...
            console.log(`   ➡️ Fiksēts iztrūkstošais ${epic} @ ${openPrice} -> TP1: ${roundTo(tp1, cfg.type)} | SL: ${roundTo(sl, cfg.type)}`);
        }
    }
