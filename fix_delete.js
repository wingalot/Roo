...
async function wipeAccount() {
    ...
    // Saglabāsim 1 katram epic
    let seenEpics = {};

    for(const p of positions) {
        if(!seenEpics[p.market.epic]) {
             seenEpics[p.market.epic] = true;
             console.log(`🛡️ Paturu: ${p.position.dealId} (${p.market.epic})`);
             continue; // Patur pirmo atrasto
        }

        console.log(`❌ Dzēšu lieko dublikātu ${p.position.dealId} (${p.market.epic})...`);
        try {
